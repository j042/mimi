...
nl.configure_newton("nonlinear_visco_solid", 1e-12, 1e-8, 40, True)
...
for i in range(10000):
    ...
    for j in range(20):
        sol()
        nl.configure_newton("nonlinear_visco_solid", 1e-6, 1e-8, 5, True)
        print("augumenting")
        print()
        if ni.gap_norm() < 1e-6:
            ...
            break
    print("final solve!")
    nl.configure_newton("nonlinear_visco_solid", 1e-7, 1e-8, 20, True)
    nl.update_contact_lagrange()
    scene.coefficient = 0.0
    c_sol()
    rel, ab = nl.newton_final_norms("nonlinear_visco_solid")

    nl.configure_newton("nonlinear_visco_solid", 1e-8, 1e-10, 3, False)
    scene.coefficient = coe
    adv()
    show()
