...
class PyNonlinearViscoSolid : public PyNonlinearSolid {
public:
  using Base_ = PyNonlinearSolid;
  ...
  virtual void SetMaterial(const MaterialPointer_& material) {
    MIMI_FUNC()
    if (!material->IsRateDependent()) {
      mimi::utils::PrintAndThrowError(material->Name(),
  ...
  virtual void Setup(const int nthreads = -1) {
    ...
    x = x_ref;
    x_dot = 0.0; // we always initialize v with 0. If this isn't desired, change
                 // here.
    ...
    auto& nonlinear_stiffness_precomputed =
        disp_fes.precomputed_["nonlinear_visco_stiffness"];
    ...
    auto nonlinear_solid_integ =
        std::make_shared<mimi::integrators::NonlinearViscoSolid>(
