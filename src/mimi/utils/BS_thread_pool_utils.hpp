 ...
 * pool library. This header file contains independent utility classes that are
 * part of the library, but are not needed to use the thread pool itself.
 */
...
public:
  ...
  // The copy constructor and copy assignment operator are deleted. The
  // signaller works using a promise, which cannot be copied.
...
public:
   ...
   *
   * @param stream The output stream to print to. The default value is
   * `std::cout`.
   */
  explicit synced_stream(std::ostream& stream = std::cout)
      : out_stream(stream) {}

  // The copy and move constructors and assignment operators are deleted. The
  // synced stream uses a mutex, which cannot be copied or moved.
   ...
   * exclusively use the same `synced_stream` object to print.
   *
  ...
  /**
   * @brief A stream manipulator to pass to a `synced_stream` (an explicit cast
   * of `std::endl`). Prints a newline character to the stream, and then flushes
   * it. Should only be used if flushing is desired, otherwise a newline
  ...
  /**
   * @brief A stream manipulator to pass to a `synced_stream` (an explicit cast
   * of `std::flush`). Used to flush the stream.
   */
...
public:
  ...
  /**
   * @brief Get the number of milliseconds that have elapsed since the object
   * was constructed or since `start()` was last called, but keep the timer
   * ticking.
   *
  ...
  /**
   * @brief Start (or restart) measuring time. Note that the timer starts
   * ticking as soon as the object is created, so this is only necessary if we
   * want to restart the clock later.
   */
  void start() { start_time = std::chrono::steady_clock::now(); }

  /**
   * @brief Stop measuring time and store the elapsed time since the object was
   * constructed or since `start()` was last called.
   */
