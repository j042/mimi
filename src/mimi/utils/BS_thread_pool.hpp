 ...
 *
 * @brief BS::thread_pool: a fast, lightweight, and easy-to-use C++17 thread
 * pool library. This header file contains the main thread pool class and some
 * additional classes and definitions. No other files are needed in order to use
...
/**
 * @brief A convenient shorthand for the type of
 * `std::thread::hardware_concurrency()`. Should evaluate to unsigned int.
 */
using concurrency_t =
...
#ifdef BS_THREAD_POOL_ENABLE_PRIORITY
/**
 * @brief A type used to indicate the priority of a task. Defined to be an
...
/**
 * @brief A type returned by `BS::this_thread::get_index()` which can optionally
 * contain the index of a thread, if that thread belongs to a `BS::thread_pool`.
 * Otherwise, it will contain no value.
 */
using optional_index = std::optional<size_t>;

/**
 * @brief A type returned by `BS::this_thread::get_pool()` which can optionally
 * contain the pointer to the pool that owns a thread, if that thread belongs to
 * a `BS::thread_pool`. Otherwise, it will contain no value.
 */
using optional_pool = std::optional<thread_pool*>;

/**
 * @brief A helper class to store information about the index of the current
 * thread.
 */
...
public:
   ...
   * `BS::thread_pool` object, it will have an index from 0 to
   * `BS::thread_pool::get_thread_count() - 1`. Otherwise, for example if this
   * thread is the main thread or an independent `std::thread`, `std::nullopt`
   * will be returned.
   *
   * @return An `std::optional` object, optionally containing a thread index.
   * Unless you are 100% sure this thread is in a pool, first use
   * `std::optional::has_value()` to check if it contains a value, and if so,
   * use `std::optional::value()` to obtain that value.
   */
...
/**
 * @brief A helper class to store information about the thread pool that owns
 * the current thread.
 */
...
public:
  /**
   * @brief Get the pointer to the thread pool that owns the current thread. If
   * this thread belongs to a `BS::thread_pool` object, a pointer to that object
   * will be returned. Otherwise, for example if this thread is the main thread
   * or an independent `std::thread`, `std::nullopt` will be returned.
   *
   * @return An `std::optional` object, optionally containing a pointer to a
   * thread pool. Unless you are 100% sure this thread is in a pool, first use
   * `std::optional::has_value()` to check if it contains a value, and if so,
   * use `std::optional::value()` to obtain that value.
   */
...
/**
 * @brief A `thread_local` object used to obtain information about the index of
 * the current thread.
 */
inline thread_local thread_info_index get_index;

/**
 * @brief A `thread_local` object used to obtain information about the thread
 * pool that owns the current thread.
 */
...
public:
  ...
  // The copy constructor and copy assignment operator are deleted. The elements
  // stored in a `multi_future` are futures, which cannot be copied.
  ...
  /**
   * @brief Check how many of the futures stored in this `multi_future` are
   * ready.
   *
  ...
  /**
   * @brief Wait for all the futures stored in this `multi_future`, but stop
   * waiting after the specified duration has passed. This function first waits
   * for the first future for the desired duration. If that future is ready
   * before the duration expires, this function waits for the second future for
   * whatever remains of the duration. It continues similarly until the duration
   * expires.
   *
  ...
  /**
   * @brief Wait for all the futures stored in this `multi_future`, but stop
   * waiting after the specified time point has been reached. This function
   * first waits for the first future until the desired time point. If that
   * future is ready before the time point is reached, this function waits for
   * the second future until the desired time point. It continues similarly
   * until the time point is reached.
   *
...
public:
  ...
  /**
   * @brief Construct a new thread pool. The number of threads will be the total
   * number of hardware threads available, as reported by the implementation.
   * This is usually determined by the number of cores in the CPU. If a core is
   * hyperthreaded, it will count as two threads.
   */
   ...
   */
  explicit thread_pool(const concurrency_t num_threads)
   ...
   * function.
   *
   * @param init_task An initialization function to run in each thread before it
   * starts to execute any submitted tasks. The function must take no arguments
   * and have no return value. It will only be executed exactly once, when the
   * thread is first constructed.
   */
  explicit thread_pool(const std::function<void()>& init_task)
      : thread_pool(0, init_task) {}

  /**
   * @brief Construct a new thread pool with the specified number of threads and
   ...
   * destroys all threads. Note that if the pool is paused, then any tasks still
   * in the queue will never be executed.
   */
  ~thread_pool() {
    wait();
    destroy_threads();
  ...
  /**
   * @brief Get a vector containing the underlying implementation-defined thread
   * handles for each of the pool's threads, as obtained by
   * `std::thread::native_handle()`. Only enabled if
   * `BS_THREAD_POOL_ENABLE_NATIVE_HANDLES` is defined.
   *
   ...
   * pool's threads, as obtained by `std::thread::get_id()`.
   *
  ...
  /**
   * @brief Check whether the pool is currently paused. Only enabled if
   * `BS_THREAD_POOL_ENABLE_PAUSE` is defined.
   *
   ...
   * running until they are finished. Only enabled if
   * `BS_THREAD_POOL_ENABLE_PAUSE` is defined.
   */
  void pause() {
    const std::scoped_lock tasks_lock(tasks_mutex);
   ...
   * running will not be affected, but any tasks still waiting in the queue will
   * be discarded, and will never be executed by the threads. Please note that
   * there is no way to restore the purged tasks.
   */
  void purge() {
    const std::scoped_lock tasks_lock(tasks_mutex);
    while (!tasks.empty())
   ...
   * task queue, with the specified priority. To push a function with arguments,
   * enclose it in a lambda expression. Does not return a future, so the user
   * must use `wait()` or some other method to ensure that the task finishes
   * executing, otherwise bad things will happen.
   *
   * @tparam F The type of the function.
   * @param task The function to push.
   * @param priority The priority of the task. Should be between -32,768 and
   * 32,767 (a signed 16-bit integer). The default is 0. Only enabled if
   ...
   * submitting each block separately to the queue, with the specified priority.
   * The block function takes two arguments, the start and end of the block, so
   * that it is only called only once per block, but it is up to the user make
   * sure the block function correctly deals with all the indices in each block.
   * Does not return a `multi_future`, so the user must use `wait()` or some
   * other method to ensure that the loop finishes executing, otherwise bad
   * things will happen.
   *
   ...
   * loop will iterate from `first_index` to `(index_after_last - 1)` inclusive.
   * In other words, it will be equivalent to `for (T i = first_index; i <
   * index_after_last; ++i)`. Note that if `index_after_last <= first_index`, no
   * blocks will be submitted.
   * @param block A function that will be called once per block. Should take
   * exactly two arguments: the first index in the block and the index after the
   * last index in the block. `block(start, end)` should typically involve a
   * loop of the form `for (T i = start; i < end; ++i)`.
   * @param num_blocks The maximum number of blocks to split the loop into. The
   * default is 0, which means the number of blocks will be equal to the number
   * of threads in the pool.
   * @param priority The priority of the tasks. Should be between -32,768 and
   * 32,767 (a signed 16-bit integer). The default is 0. Only enabled if
   * `BS_THREAD_POOL_ENABLE_PRIORITY` is defined.
   */
  template<typename T, typename F>
  void
  detach_blocks(const T first_index,
                const T index_after_last,
                F&& block,
                const size_t num_blocks = 0 BS_THREAD_POOL_PRIORITY_INPUT) {
    if (index_after_last > first_index) {
      const blocks blks(first_index,
                        index_after_last,
                        num_blocks ? num_blocks : thread_count);
      for (size_t blk = 0; blk < blks.get_num_blocks(); ++blk) {
        detach_task(
            [block = std::forward<F>(block),
             start = blks.start(blk),
             end = blks.end(blk),
             blk] { block(start, end, blk); } BS_THREAD_POOL_PRIORITY_OUTPUT);
      }
    }
  }
   ...
   * @brief Parallelize a loop by automatically splitting it into blocks and
   * submitting each block separately to the queue, with the specified priority.
   * The loop function takes one argument, the loop index, so that it is called
   * many times per block. Does not return a `multi_future`, so the user must
   * use `wait()` or some other method to ensure that the loop finishes
   * executing, otherwise bad things will happen.
   *
   * @tparam T The type of the indices. Should be a signed or unsigned integer.
   * @tparam F The type of the function to loop through.
   ...
   */
  template<typename T, typename F>
  void detach_loop(const T first_index,
                   const T index_after_last,
                   F&& loop,
                   const size_t num_blocks = 0 BS_THREAD_POOL_PRIORITY_INPUT) {
    if (index_after_last > first_index) {
  ...
  /**
   * @brief Submit a sequence of tasks enumerated by indices to the queue, with
   * the specified priority. Does not return a `multi_future`, so the user must
   * use `wait()` or some other method to ensure that the sequence finishes
   * executing, otherwise bad things will happen.
   *
   * @tparam T The type of the indices. Should be a signed or unsigned integer.
   * @tparam F The type of the function used to define the sequence.
   * @param first_index The first index in the sequence.
   * @param index_after_last The index after the last index in the sequence. The
   * sequence will iterate from `first_index` to `(index_after_last - 1)`
   ...
   */
  template<typename T, typename F>
  void detach_sequence(const T first_index,
                       const T index_after_last,
  ...
  /**
   * @brief Reset the pool with the total number of hardware threads available,
   * as reported by the implementation. Waits for all currently running tasks to
   * be completed, then destroys all threads in the pool and creates a new
   * thread pool with the new number of threads. Any tasks that were waiting in
   * the queue before the pool was reset will then be executed by the new
   * threads. If the pool was paused before resetting it, the new pool will be
   * paused as well.
   */
  void reset() {
    reset(0, [] {});
  ...
  /**
   * @brief Reset the pool with a new number of threads. Waits for all currently
  ...
  /**
   * @brief Reset the pool with the total number of hardware threads available,
   * as reported by the implementation, and a new initialization function. Waits
   * for all currently running tasks to be completed, then destroys all threads
   * in the pool and creates a new thread pool with the new number of threads
   * and initialization function. Any tasks that were waiting in the queue
   * before the pool was reset will then be executed by the new threads. If the
   * pool was paused before resetting it, the new pool will be paused as well.
   *
   * @param init_task An initialization function to run in each thread before it
   * starts to execute any submitted tasks. The function must take no arguments
   * and have no return value. It will only be executed exactly once, when the
   * thread is first constructed.
   */
  void reset(const std::function<void()>& init_task) { reset(0, init_task); }

  /**
   * @brief Reset the pool with a new number of threads and a new initialization
   * function. Waits for all currently running tasks to be completed, then
   * destroys all threads in the pool and creates a new thread pool with the new
   * number of threads and initialization function. Any tasks that were waiting
   * in the queue before the pool was reset will then be executed by the new
   * threads. If the pool was paused before resetting it, the new pool will be
   * paused as well.
   *
   * @param num_threads The number of threads to use.
   ...
   * lambda expression. If the function has a return value, get a future for the
   * eventual returned value. If the function has no return value, get an
   ...
   * that it is only called only once per block, but it is up to the user make
   * sure the block function correctly deals with all the indices in each block.
   * Returns a `multi_future` that contains the futures for all of the blocks.
   *
   * @tparam T The type of the indices. Should be a signed or unsigned integer.
   * @tparam F The type of the function to loop through.
   ...
   * In other words, it will be equivalent to `for (T i = first_index; i <
   * index_after_last; ++i)`. Note that if `index_after_last <= first_index`, no
   * blocks will be submitted, and an empty `multi_future` will be returned.
   * @param block A function that will be called once per block. Should take
   * exactly two arguments: the first index in the block and the index after the
   * last index in the block. `block(start, end)` should typically involve a
   * loop of the form `for (T i = start; i < end; ++i)`.
   ...
   * 32,767 (a signed 16-bit integer). The default is 0. Only enabled if
   * `BS_THREAD_POOL_ENABLE_PRIORITY` is defined.
   * @return A `multi_future` that can be used to wait for all the blocks to
   * finish. If the block function returns a value, the `multi_future` can also
   * be used to obtain the values returned by each block.
   */
  template<typename T,
           typename F,
           typename R = std::invoke_result_t<std::decay_t<F>, T, T>>
  [[nodiscard]] multi_future<R>
  submit_blocks(const T first_index,
                const T index_after_last,
                F&& block,
                const size_t num_blocks = 0 BS_THREAD_POOL_PRIORITY_INPUT) {
    if (index_after_last > first_index) {
      ...
      for (size_t blk = 0; blk < blks.get_num_blocks(); ++blk)
        future.push_back(submit_task([block = std::forward<F>(block),
                                      start = blks.start(blk),
                                      end = blks.end(blk)] {
   ...
   * submitting each block separately to the queue, with the specified priority.
   * The loop function takes one argument, the loop index, so that it is called
   * many times per block. It must have no return value. Returns a
   * `multi_future` that contains the futures for all of the blocks.
   *
   * @tparam T The type of the indices. Should be a signed or unsigned integer.
   * @tparam F The type of the function to loop through.
   * @param first_index The first index in the loop.
   ...
   * In other words, it will be equivalent to `for (T i = first_index; i <
   * index_after_last; ++i)`. Note that if `index_after_last <= first_index`, no
   * tasks will be submitted, and an empty `multi_future` will be returned.
   * @param loop The function to loop through. Will be called once per index,
   * many times per block. Should take exactly one argument: the loop index. It
  ...
  submit_loop(const T first_index,
              const T index_after_last,
              F&& loop,
              const size_t num_blocks = 0 BS_THREAD_POOL_PRIORITY_INPUT) {
    if (index_after_last > first_index) {
      ...
      future.reserve(blks.get_num_blocks());
      for (size_t blk = 0; blk < blks.get_num_blocks(); ++blk)
        future.push_back(submit_task([loop = std::forward<F>(loop),
                                      start = blks.start(blk),
                                      end = blks.end(blk)] {
          for (T i = start; i < end; ++i)
            loop(i);
  ...
  /**
   * @brief Submit a sequence of tasks enumerated by indices to the queue, with
   * the specified priority. Returns a `multi_future` that contains the futures
   * for all of the tasks.
   *
   * @tparam T The type of the indices. Should be a signed or unsigned integer.
   * @tparam F The type of the function used to define the sequence.
   * @tparam R The return type of the function used to define the sequence (can
   * be `void`).
   * @param first_index The first index in the sequence.
   * @param index_after_last The index after the last index in the sequence. The
   * sequence will iterate from `first_index` to `(index_after_last - 1)`
   * inclusive. In other words, it will be equivalent to `for (T i =
   * first_index; i < index_after_last; ++i)`. Note that if `index_after_last <=
   * first_index`, no tasks will be submitted, and an empty `multi_future` will
   * be returned.
   * @param sequence The function used to define the sequence. Will be called
   * once per index. Should take exactly one argument, the index.
   * @param priority The priority of the tasks. Should be between -32,768 and
   * 32,767 (a signed 16-bit integer). The default is 0. Only enabled if
   * `BS_THREAD_POOL_ENABLE_PRIORITY` is defined.
   * @return A `multi_future` that can be used to wait for all the tasks to
   * finish. If the sequence function returns a value, the `multi_future` can
   * also be used to obtain the values returned by each task.
   */
  template<typename T,
           typename F,
           typename R = std::invoke_result_t<std::decay_t<F>, T>>
  [[nodiscard]] multi_future<R>
  submit_sequence(const T first_index,
                  const T index_after_last,
                  F&& sequence BS_THREAD_POOL_PRIORITY_INPUT) {
    if (index_after_last > first_index) {
      multi_future<R> future;
   ...
   * of the queue. Only enabled if `BS_THREAD_POOL_ENABLE_PAUSE` is defined.
   */
  void unpause() {
    {
      const std::scoped_lock tasks_lock(tasks_mutex);
...
// Macros used internally to enable or disable pausing in the waiting and worker
// functions.
#ifdef BS_THREAD_POOL_ENABLE_PAUSE
...
#define BS_THREAD_POOL_PAUSED_OR_EMPTY tasks.empty()
   ...
   * which would result in a deadlock. Only enabled if
   * `BS_THREAD_POOL_ENABLE_WAIT_DEADLOCK_CHECK` is defined.
   */
  void wait() {
#ifdef BS_THREAD_POOL_ENABLE_WAIT_DEADLOCK_CHECK
  ...
  /**
   * @brief Wait for tasks to be completed, but stop waiting after the specified
   * duration has passed.
   *
   * @tparam R An arithmetic type representing the number of ticks to wait.
   * @tparam P An `std::ratio` representing the length of each tick in seconds.
   ...
   * @tparam D An `std::chrono::duration` type used to indicate the time point.
   * @param timeout_time The time point at which to stop waiting.
   * @return `true` if all tasks finished running, `false` if the time point was
   * reached but some tasks are still running.
   *
   * @throws `wait_deadlock` if called from within a thread of the same pool,
   * which would result in a deadlock. Only enabled if
   * `BS_THREAD_POOL_ENABLE_WAIT_DEADLOCK_CHECK` is defined.
...
private:
   ...
   *
   * @param init_task An initialization function to run in each thread before it
   * starts to execute any submitted tasks.
   */
   ...
   *
   * @param num_threads The parameter passed to the constructor or `reset()`. If
   * the parameter is a positive number, then the pool will be created with this
   ...
   * obtained from `std::thread::hardware_concurrency()`. If the latter returns
   * zero for some reason, then the pool will be created with just one thread.
   * @return The number of threads to use for constructing the pool.
   */
  [[nodiscard]] static concurrency_t
  determine_thread_count(const concurrency_t num_threads) {
    if (num_threads > 0)
      return num_threads;
   ...
   * until it is notified by `detach_task()` that a task is available, and then
   * retrieves the task from the queue and executes it. Once the task finishes,
   * the worker notifies `wait()` in case it is waiting.
   *
   * @param idx The index of this thread.
   * @param init_task An initialization function to run in this thread before it
   * starts to execute any submitted tasks.
   */
  ...
  public:
    /**
     * @brief Construct a new task with an assigned priority by copying the
     * task.
     *
   ...
   * `true`, the workers temporarily stop retrieving new tasks out of the queue,
  ...
  /**
   * @brief A flag indicating that `wait()` is active and expects to be notified
