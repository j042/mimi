...
public:
  ...
  virtual void Prepare(const int quadrature_order = -1) {
    ...
    boundary_d_shapes.resize(n_boundary_patches);
    ...
    auto precompute_trans_weights = [&](const int marked_b_el_begin,
      ...
      for (int i{marked_b_el_begin}; i < marked_b_el_end; ++i) {
        ...
        auto& lagrange = augmented_lagrange_multipliers[i_mbe];
        ...
        Base_::boundary_element_vectors_->operator[](i_mbe).SetSize(
            i_b_el->GetDof() * dim_);
        ...
        for (int j{}; j < n_quad; ++j) {
          ...
          results[j].SetSize(boundary_para_dim_, dim_);
        }
      }
    };
  ...
  virtual void AssembleFaceResidual(const mfem::Vector& current_x) {
    ...
    auto assemble_face_residual = [&](const int begin,
      ...
      for (int i{begin}; i < end; ++i) {
        ...
        const int i_patch_id = i_b_el->GetPatch();
        const auto& i_shapes = boundary_shapes[i_patch_id];
        const auto& i_d_shapes = boundary_d_shapes[i_patch_id];
        ...
        current_quad_derivatives.SetSize(n_dof, boundary_para_dim_);
        ...
        for (int q{}; q < int_rule.GetNPoints(); ++q) {
          ...
          // get traction - t_factor should come out as positive value
