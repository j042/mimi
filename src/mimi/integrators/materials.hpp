...
public:
  ...
  virtual void Setup(TemporaryData& tmp) {
    MIMI_FUNC()
  ...
  virtual void EvaluatePK1(const MaterialStatePtr_& state,
    ...
    // P = det(F) * sigma * F^-T
    mfem::MultABt(sigma, tmp.FInv(), P);
    P *= tmp.Det();
  }

  /// state accumulating version
  virtual void Accumulate(TemporaryData& tmp, MaterialStatePtr_& state) {
    MIMI_FUNC()
...
public:
  ...
  virtual void Accumulate(MaterialStatePtr_& state,
                          ...
                          mfem::DenseMatrix& sigma) const {
    MIMI_FUNC()
    PlasticSress<true>(state, tmp, sigma);
  };
  ...
  public:
    ...
    void PlasticStress(std::conditional_t<accumulate,
      ...
      if (residual(0.0) > tolerance) {
        ...
        const double upper_bound =
            (q
             - hardening_->Evaluate(eqps_old, eqps_rate).GetValue()
                   / (3.0 * G_));
        const double delta_eqps = mimi::solvers::ScalarSolve(residual,
                                                             0.0,
                                                             lower_bound,
    ...
    void PlasticStrainRateAndTemperatureRate(const mfem::DenseMatrix& F_dot,
      ...
      if (dim_ == 2) {
        ...
        if (work < 0.0) {
          ...
          temperature_rate =
              heat_fraction_ * work / (density_ * specific_heat_);
        }
